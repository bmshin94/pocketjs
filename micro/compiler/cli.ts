...
//   --receipt <n>    first receipt frame (default 240)
//   --receipt-every <n> receipt interval (default 600)
...
import { validatePspLayout } from "./psp-layout.ts";
...
const positional = argv.slice(1).filter((a, i, all) => !a.startsWith("--") && !(i > 0 && ["--out", "--tape", "--receipt", "--receipt-every"].includes(all[i - 1])));
...
// A receipt identifies runtime/host code and input tape as well as the app.
const nativeHash = createHash("sha256");
function hashSource(path: string): void {
  if (!existsSync(path)) return;
  if (statSync(path).isDirectory()) {
    for (const entry of readdirSync(path).sort()) hashSource(join(path, entry));
  } else nativeHash.update(path.replace(ROOT, "")).update(readFileSync(path));
}
for (const dir of ["engine/core", "engine/crates/pocket-micro", "hosts/psp", "hosts/psp-micro"]) {
  for (const source of ["src", "build.rs", "Cargo.toml", "Cargo.lock", ".cargo", "targets"]) hashSource(join(ROOT, dir, source));
}
const runtimeHash = nativeHash.digest("hex");
const buildHash = createHash("sha256").update(rust).update(assets.pak).update(runtimeHash)
  .update(flag("--tape") ?? "").update(flag("--receipt") ?? "240").update(flag("--receipt-every") ?? "600").digest("hex").slice(0, 16);
await Bun.write(
...
if (existsSync(fragment)) {
  ...
  await Bun.write(generatedToml, `# GENERATED by micro/compiler/cli.ts\ntitle = ${JSON.stringify(program.title + " (Micro)")}\n`);
}
...
const env: Record<string, string> = {
  ...
  POCKETJS_OFFLOAD_SLOT: program.windows.length ? createHash("sha256").update(appName).digest("hex").slice(0, 16) : "",
  // The compiled app.
  ...
  POCKET_MICRO_RECEIPT_EVERY: flag("--receipt-every") ?? "600",
};
...
validatePspLayout(new Uint8Array(await Bun.file(prx).arrayBuffer()));
