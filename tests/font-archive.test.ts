...
test("a full pinned source cache backs off every rejected batch", async () => {
  const { createFontArchive } = await import("../framework/src/fonts.ts");
  const { createOffloadClient } = await import("../framework/src/offload.ts");
  const { runServicePumps } = await import("../framework/src/services.ts");
  const { archiveProvider } = await import(
    "./helpers/font-archive-provider.ts"
  );
  const bytes = await bakeFontArchive({
    font: "assets/fonts/NotoSansCJK-Demo.otf",
    slots: [2],
  ...
  const provider = archiveProvider(bytes),
    client = createOffloadClient(provider.ops);
  const host = {
    fontStreamConfigure: () => true,
    fontStreamRequests: () => JSON.stringify([[1, 2, 0x4e00]]),
    fontStreamCommit: () => 0,
    fontStreamStats: () => "{}",
  };
  const archive = createFontArchive(
    { path: "font.pjfa", slots: [2], capacity: 1 },
    host as any,
    client,
  );
  try {
